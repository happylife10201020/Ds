...
int main(void) {
    ...
    if (numberOfExponent1 == 0) {
        ...
        data1[0] = calloc(2, sizeof(int));
        if (data1[0] == NULL) {
            fprintf(stderr, "Error allocating memory for data1[0]\n");
            exit(1);
        }
    ...
    if (numberOfExponent2 == 0) {
        ...
        data2[0] = calloc(2, sizeof(int));
        if (data2[0] == NULL) {
            fprintf(stderr, "Error allocating memory for data2[0]\n");
            exit(1);
        }
    }else {
...
double addPolyArrayBasic(FILE * output,int **data1, int **data2, int n, int m) {
    ...
    for (int i = 0; i < n; i++) {
        if (data1[i][0] != 0) {
            ...
            if (isFirst != 1 && data1[i][0] > 0) fprintf(output, " + ");
            fprintf(output, "%dx^%d", data1[i][0], data1[i][1]);
            isFirst = 0;
        }
    }
    ...
    for (int i = 0; i < m; i++) {
        if (data2[i][0] != 0) {
            ...
            if (isFirst != 1 && data2[i][0] > 0) fprintf(output, " + ");
            fprintf(output, "%dx^%d", data2[i][0], data2[i][1]);
            isFirst = 0;
        }
    }
    if (isFirst) fprintf(output, "0");
...
double addPolyArrayOptimized(FILE * output, int ** data1,  int ** data2, /**첫번째 다항식 항의 수*/int n, /**두번째 다항식 항의 수*/int m) {
    ...
    for (int i = 0; i<OTHER_REPEAT; i++) {
        ...
        //첫 번째 반복일 때 출력
        if (i == 0) {
            int isFirstinThisloop = 1;
            for (int l = 0; l<n; l++) {
                if (sumArray[l][0] == 0) continue;
                if (sumArray[l][0] > 0 && isFirstinThisloop != 1) {
                    fprintf(output, " + ");
                }
                fprintf(output, "%dx^%d", sumArray[l][0], sumArray[l][1]);
                isFirstinThisloop = 0;
            }
            if (isFirstinThisloop) fprintf(output, "0");
            fprintf(output, "\n");
    ...
    if (PRINT_ALL) {
        ...
        for (int i = 0; i<n+m; i++) {
            ...
            fprintf(output, "%dx^%d", resultArray[i][0], resultArray[i][1]);
            isFirst = 0;
        }
        if (isFirst) fprintf(output, "0");
        fprintf(output, "\n");
...
double addPolyArrayLinkedList(FILE * output, int ** data1,  int ** data2, /**첫번째 다항식 항의 수*/int n, /**두번째 다항식 항의 수*/int m) {
    ...
    for (int i = 0; i<n;i++) {
        ...
        if (isFirst != 1 && temp->coef > 0 ) fprintf(output, " + ");
        fprintf(output, "%dx^%d", temp->coef, temp->exp);
        isFirst = 0;
    }
    if (isFirst) fprintf(output, "0");
